...
3. SSE run → MCP mapping (progress vs poll): **Answered:** Poll. Prefer a durable-job pattern (`start_research_run` + `poll_run`) over streaming: the SDK does have (experimental) task support and progress notifications, but a poll tool matched to the existing `runBroker` is the thinner adapter. Two guards, both already provided by the HTTP layer, must be reused rather than re-implemented: (a) concurrent runs — `/sessions/:id/run` rejects a second live run with 409 and `runBroker.start` throws `RunAlreadyActiveError`; the adapter only maps these to an MCP error. (b) completion — `runBroker` retains completed run frames for only ~30s (`DEFAULT_COMPLETED_RETENTION_MS`), so a late `poll_run` must source the terminal result from durable stores (session history, notebook, cost ledger) and explicitly reconcile "completed but expired", not report a fresh "no run". Budget-blocked runs return HTTP 403 with `reason:"budget"`; map that to a distinct MCP error so clients don't mistake a hard spend cap for a retryable failure.
