...
- [x] Add a durable terminal-result record keyed by `runId` under the existing per-project run-data tree and a lookup that returns its terminal status/result after the broker's ~30s retention expires; `poll_run` must read the broker while live and the persisted record afterward. The snapshot is intentionally an atomic synchronous write for every completed local run: that small completion-path cost guarantees the record exists before broker expiry; Phase 2 does not add a second, MCP-only run path.
