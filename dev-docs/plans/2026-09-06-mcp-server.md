...
**Exit criteria (master):** an external MCP client completes a Kady research task end-to-end through MCP tools only; docs let a new client connect; CLI remains explicitly deferred with its entry point recorded (todo §3).
...
- MCP SDK upgrades are deliberate (harness pinning policy); Phase 1 records the exact resolved version.
