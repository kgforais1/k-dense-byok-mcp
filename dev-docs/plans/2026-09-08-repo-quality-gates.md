...
- **A standalone "no absolute paths" script** — rejected as redundant. The `local-home-directory-path` gitleaks rule covers it, over the tracked tree — deliberately not history, for the reason given above. Worth recording that the real near-miss during MCP Phase 2 was *runtime* output — `create_research_session` returning `session.sessionFile` — which no source scan would ever have caught. That class of leak needs a contract test, and has one.
