...
### Gitleaks configuration (three configs, three passes)
...
- **`max-lines: 1136`** — as written on 2026-09-08, set at the worst file then: `server/src/modal/manager.ts`, with `api/sessions.ts` at 892 and `api/sandbox.ts` at 879. Those three have since grown to **1467**, 1219 and 932, and the cap tracks the first of them: it lives in `server/.ratchets.json`, moves only downward, and stops at a floor of 750. The figures here are the record of the decision, not the current state — read the ratchet file for that. On the frontend the problem is worse: `components/chat-tab.tsx` is 2355 lines and `components/file-preview-panel.tsx` 1978. ~~**Adding `max-lines` to `web/eslint.config.mjs` is deferred precisely because it cannot be set at a useful value today.**~~ — **no longer deferred; shipped in PR #45.** The premise was wrong, not just the numbers: a cap pinned at the worst file is useful *because* it is ugly, since it stops the next file getting worse while demanding nothing of the ones already there. Both packages now carry a self-lowering cap (`server/.ratchets.json` at 1467, `web/.ratchets.json` at 2355, floor 750), and `ratchet:check` fails both a stale-high cap and a file that exceeds one.
...
- **Semgrep** — rejected as a generic ruleset. For TypeScript it overlaps CodeQL heavily and would mostly add duplicate findings. There *is* a real use for it, deferred rather than dismissed: custom rules encoding this repository's own invariants, which no off-the-shelf scanner knows about. Candidates, all from guardrails that already exist in prose in `AGENTS.md` and the MCP plans: an MCP tool result must never include an absolute host path; `prepareRun` must not call `reply.code`; there must be no second, MCP-only run path. Those are worth writing as rules when there are enough of them to justify the job. A reviewer made a sharper point about the framing: two of those three are not Semgrep-shaped at all. "No absolute host path in a tool result" is a property of *runtime output*, which no static rule can see — `server/test/mcp-server-tools.test.ts:122` pins the one known instance, but `get_session_history` and `poll_run` have no such assertion. "No second, MCP-only run path" wants a test driving `run_already_active` through the MCP adapter to prove it shares `beginRun`'s ownership and billing. Only "`prepareRun` must not call `reply.code`" is structural. Deferring the *job* is right; deferring the two tests is not, and they are carried into Phase 3 of the MCP plan rather than left here.
  ...
  **Resolved 2026-09-22 — Semgrep rejected outright, the one rule shipped elsewhere.** Re-examining the candidate list left exactly one Semgrep-shaped rule, and two further ideas were measured and died on the data: `process.env` is read 31 times across 12+ backend files, so "centralise env access" is a refactor rather than a gate, and there are only 4 `fetch` call sites, too few to justify a rule about timeouts. A second scanner, config, annotation vocabulary and version pin for one rule is a bad trade. The rule is now `no-restricted-syntax` in `server/eslint.config.mjs`, inside the lint that already runs in CI, with `server/test/lint-rules.test.ts` linting synthetic sources through the real config so the rule is checked by behaviour rather than by its presence in the file. It guards the signature rather than the use site: a parameter named `reply`/`res`/`response`, or anything typed `FastifyReply` anywhere inside, since renaming, aliasing, destructuring or forwarding the parameter all still couple `prepareRun` to HTTP and all have to come through the parameter list first. Member access on those names is matched too, for a reply reached from module scope. Covers `prepareRun` as a declaration or as a const arrow.
...
- **A standalone "no absolute paths" script** — rejected as redundant *for the case it was proposed to catch*. The `local-home-directory-path` gitleaks rule covers committed home-directory paths — `/Users/<name>/`, `/home/<name>/`, `C:\Users\<name>\` — over the tracked tree, deliberately not history, for the reason given above. It is not a general no-absolute-paths check and is not meant to be: `/usr/local/bin`, `/tmp/...` and `/etc/...` are ordinary and are not flagged. Separately, the real near-miss during MCP Phase 2 was *runtime* output — `create_research_session` returning `session.sessionFile` — which no source scan of any kind would have caught. That class of leak needs a contract test. One exists for that tool at `server/test/mcp-server-tools.test.ts:122`; extending it across every MCP tool result is carried into Phase 3.
