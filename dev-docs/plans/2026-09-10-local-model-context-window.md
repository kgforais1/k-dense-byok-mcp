...
`contextWindow: 32_768`. That number is not what either local server reports,
and it is below what Kady's own prompt needs.

The evidence behind this plan lives in a companion file:
...
above. The findings file states the chain and its one unproven link. Phase 4
exists to test it.
