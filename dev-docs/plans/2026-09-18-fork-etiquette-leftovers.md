...
Measured 2026-09-18 on `main` at `5ae0604`. Two claims in todo §6 are stale (flagged inline); per the repo source-of-truth order, measured truth wins and the implementing PR corrects the todo.
...
2. **Fork notices**: present in `README.md:9-26` and `docs/installation.md`. Absent from the other 27 files under `docs/*.md`, including `docs/limitations.md` (which routes Windows issues to this fork's tracker at `:136` but never states the relationship — the todo's claim that `limitations.md` says it is stale).
3. **Skills figure**: all three prose sites now say **149** (`README.md:89`, `docs/basic-usage.md:20`, `docs/codebase-summary.md:12`) — the 140+ inconsistency recorded in the todo is stale; the figure is consistent but unverifiable locally. Skills sync from `K-Dense-AI/scientific-agent-skills` at whatever revision arrives (`server/src/agent/config.ts:42`, `KADY_SKILLS_REPO` override), so no local count can settle it. Workflows 326 and databases 229 were verified against `web/src/data/workflows.json` and `databases.json` — keep, but they currently have no check behind them.
...
- **Skills figure: drop the number, name the source.** Reword the three prose sites to point at the default catalogue repo plus the `KADY_SKILLS_REPO` override instead of asserting a count — the todo's "either name that revision or drop the figure", and naming a revision of a repo we do not control is the weaker option.
...
docs/<24 remaining user-facing docs>         # add one-line fork notice
scripts/docs-check.mjs                       # counts assertion for 326/229
dev-docs/todo.md                             # §6: "Planned in [...]" pointer + fix 2 stale claims (implementing PR)
```
...
- [ ] Reword the three "149 skills" prose sites to name the catalogue source instead of a count.
...
- [ ] Add the one-line notice to all 27 user-facing `docs/*.md` files that lack one (every file except `installation.md`); skip `docs/development/*`.
...
- [ ] Implementing PR adds the "Planned in" pointer to todo §6, fixes the two stale claims (limitations.md notice, 140+ inconsistency), moves this plan to `completed/`, sets final status, and records in `CHANGELOG.md` and/or `maintenance-log.md` per the archive lifecycle.
...
| No unverifiable skills count in prose | `grep -rn "149" README.md docs/basic-usage.md docs/codebase-summary.md` returns nothing |
| Every user-facing doc names the fork | `grep -rL "Fork note" docs/*.md` lists only `installation.md` (which has its own notice wording) |
